function [sizeout , deltaout , tracking] = fingrmeasure
             ...
             if budtnumber{i+1}(j1)>0
                 if budtnumber{i+1}(j1)==budtnumber{i}(j)
                     ...
                     if (diff1>=low_be)&&(diff1<=high_be)&&(diff2>=low_db)&&(diff2<=high_db)
                         ...
                         trackmotherid{i}=[trackmotherid{i} , cid];
                         trackbudid{i}=[trackbudid{i} , bid];
                         if (segmentation.tcells1(cid).birthFrame>0 )
                             trackgeneration{i}=[trackgeneration{i} , find(segmentation.tcells1(cid).daughterList==bid)];
                             trackbirth{i}=[trackbirth{i} , segmentation.tcells1(cid).birthFrame*framet-framet]; 
                         else
                             if (segmentation.tcells1(cid).detectionFrame==1)
                                 trackgeneration{i}=[trackgeneration{i} , -1];
                                 trackbirth{i}=[trackbirth{i} , 0];
                             else
                                 trackgeneration{i}=[trackgeneration{i} , -1];
                                 trackbirth{i}=[trackbirth{i} , -1];
                             end;
                         end;
%                          if (segmentation.tcells1(cid).detectionFrame==1)&&(segmentation.tcells1(cid).birthFrame==0)
%                              matrixvreme1=[matrixvreme1 , (i-1)*framet];
%                              vreme=[vreme , 0];                            
%                              vrmgen=[vrmgen , diff1 + diff2];
%                          end;
%                          if (segmentation.tcells1(cid).birthFrame>0 )
%                              generation=[generation , find(segmentation.tcells1(cid).daughterList==bid)];
%                              grgen=[grgen , diff1 + diff2];
%                              matrixvreme1=[matrixvreme1 , (i-1)*framet];
%                              vreme=[vreme , segmentation.tcells1(cid).birthFrame*framet-framet];                            
%                              vrmgen=[vrmgen , diff1 + diff2];
%                          end;
                     end;
                 else
...
% figure;
% vreme=vreme((matrixvreme1>650)&(matrixvreme1<950));
% vrmgen=vrmgen((matrixvreme1>650)&(matrixvreme1<950));
% [ddddd , ppppppp]=ttest2(vrmgen(vreme<=330) , vrmgen(vreme>330))
% ranksum(vrmgen(vreme<=330) , vrmgen(vreme>330))
% scatter(vreme , vrmgen);
%  set(gca,'FontSize',20)
%         xlabel('Birth time');
%         ylabel('Cell growth (au)');
%         title('Cell growth during the time lapse');
% figure;
% scatter(generation , grgen);
%  set(gca,'FontSize',20)
%         xlabel('Generation');
%         ylabel('Cell growth (au)');
%         title('Cell growth during aging');
%% out
size1.self=selfsize;
delta1.self=selfdelta;
size1.mother=mothersize;
delta1.mother=motherdelta;
size1.child=childsize;
delta1.child=childelta;
size1.all=motherchildsize;
delta1.all=motherchildelta;
sizeout=size1;
deltaout=delta1;
track1.self=trackselfid;
track1.mother=trackmotherid;
track1.child=trackbudid;
track1.generation=trackgeneration;
track1.birth=trackbirth;
tracking=track1;
end
