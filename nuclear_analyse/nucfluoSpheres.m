function [celltfluo , cellcfluo , cellnfluo ,  cellnenrfluo , cellarea , celltime] = nucfluoSpheres(resolution ,chanelfluo)
...
for j=celapp(ii):length(segmentation.tcells1(i).Obj)    
    ...
    lll=segmentation.tcells1(i).Obj(j).area;
    totarea=segmentation.tcells1(i).Obj(j).area;
    totarea=(4*sqrt(totarea*totarea*totarea/pi))/3;
         if segmentation.tcells1(i).Obj(j).Mean.n>0
             ...
             cyt=(tot*lll-nuc*nucarea)/(lll-nucarea);
             nucenr=(nuc-cyt)/tot;
         else
             nuc=0;
             nucarea=0;
...
lll=segmentation.tcells1(i).Obj(j).area+segmentation.tcells1(doublets{ii}(j)).Obj(j-diff).area;
xxx=segmentation.tcells1(i).Obj(j).area;
yyy=segmentation.tcells1(doublets{ii}(j)).Obj(j-diff).area;

totarea=((4*sqrt(xxx*xxx*xxx/pi))/3)+(4*sqrt(yyy*yyy*yyy/pi))/3;


tot=(segmentation.tcells1(i).Obj(j).fluoMean(cha)*segmentation.tcells1(i).Obj(j).area+segmentation.tcells1(doublets{ii}(j)).Obj(j-diff).fluoMean(cha)*segmentation.tcells1(doublets{ii}(j)).Obj(j-diff).area)/lll;
         if segmentation.tcells1(i).Obj(j).Mean.n>0
             n=segmentation.tcells1(i).Obj(j).Mean.n;
             time=segmentation.tcells1(i).Obj(j).image;
             nt=time-segmentation.tnucleus(n).detectionFrame+1;
