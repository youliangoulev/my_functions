function [aut]=outlayers3(resolution ,chanelfluo)
...
binseuil=0.000021; %   0.000021 for 100x obj
ratior=2.5; %   1000 for 60x obj   ||   2 for 100x obj
...
for i=1:length(segmentation.tcells1)   
    if length(segmentation.tcells1(i).Obj)>1
        for j=2:length(segmentation.tcells1(i).Obj)
            ...
            deltax=segmentation.tcells1(i).Obj(j).ox-segmentation.tcells1(i).Obj(j-1).ox;
            deltay=segmentation.tcells1(i).Obj(j).oy-segmentation.tcells1(i).Obj(j-1).oy;
            edist000=[edist000 , sqrt((deltax*deltax)+(deltay*deltay))];            
...
stillin=true;    
 while stillin   
    ...
    if (sum((etime000>=mmp)&(etime000<=nnp))>=celimitn)
        ...
        [dddd,ppt3]=ttest2(edist000(compare) , edist000(comparex));
        [dddd,ppv3]=vartest2(edist000(compare) , edist000(comparex)); 
        
        
        if (ppt1>alpha)&&(ppv1>alpha)&&(ppt2>alpha)&&(ppv2>alpha)&&(ppt3>alpha)&&(ppv3>alpha)
            efluo=[efluo , efluo000(comparex)];
            esize=[esize , esize000(comparex)];
            edist=[edist , edist000(comparex)];
...
for i=1:length(esize)    
             produit=(sum(esize>=esize(i))/length(esize))*(sum(efluo>=efluo(i))/length(efluo))*(sum(edist>=edist(i))/length(edist));
             k=[k , produit];
             c=[c , produit*(1-log(produit)+log(produit)*log(produit)/2)];     
