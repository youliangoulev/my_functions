function [celltfluo , cellcfluo , cellnfluo ,  cellnenrfluo , cellvolume , celltime] = nucfluointensityv(resolution ,chanelfluo)
...
%              coefw=1-r/R;
            ...
            nucc=((nuc-cyt*difcyt)*nucarea)/((4*sqrt(nucarea*nucarea*nucarea/pi))/3);
            totc=(tot*totarea)/((4*sqrt(totarea*totarea*totarea/pi))/3);
             
             
             nucenr=(nucc)/(totc); % difcyt
         else
             nuc=0;
             nucarea=0;
             cyt=tot;
...
%              nucer=(nucer1*nucareax+nucer2*nucareay)/(nucareax+nucareay);
             nuc1v=4*sqrt(nucareax*nucareax*nucareax/pi)/3;
             nuc2v=4*sqrt(nucareay*nucareay*nucareay/pi)/3;
             totv1=4*sqrt(xxx*xxx*xxx/pi)/3;
             totv2=4*sqrt(yyy*yyy*yyy/pi)/3;  
             tot1=segmentation.tcells1(i).Obj(j).fluoMean(cha)-bgd-segmentation.tcells1(i).Obj(j).image*inc;
             tot2=segmentation.tcells1(doublets{ii}(j)).Obj(j-diff).fluoMean(cha)-bgd-segmentation.tcells1(doublets{ii}(j)).Obj(j-diff).image*inc;
             cyt1=(tot1*xxx-nucx*nucareax)/(xxx-nucareax);
             cyt2=(tot2*yyy-nucy*nucareay)/(yyy-nucareay);            
             nucc=((nucx-cyt1*difcyt)*nucareax+(nucy-cyt2*difcyt)*nucareay)/(nuc1v+nuc2v);
             totc=((tot1*xxx)+(tot2*yyy))/(totv1+totv2);
